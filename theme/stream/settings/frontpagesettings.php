...
while ($i < $slidestotal ) {
	...
	/* $setting = new admin_setting_configtext($name, $title, $description , 'Đại học Bách khoa Hà Nội', PARAM_TEXT); */
	$setting = new admin_setting_configtext($name, $title, $description, 'Đại học Bách khoa Hà Nội', 'Khoa Khoa học và công nghệ giáo dục');
    $setting->set_updatedcallback('theme_reset_all_caches');
    $page->add($setting);
    ...
    'Kiến tạo tương lai số' , PARAM_TEXT);
    $setting->set_updatedcallback('theme_reset_all_caches');
    $page->add($setting);
